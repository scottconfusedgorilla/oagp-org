...
- **Pattern-promotion watch-list** — candidates held pending more derivation/validation (revisit, don't re-decide): thingalog AI-PM/synthesis-agent seat (ships ~1–2 months; memos/2026-05-25-2000/2100); three-tier permission composition (2 derivations, one short; arguably substrate-MCP-surface-shape → catdef/memodef family); **seat-capability-manifest** (convention decided 2026-05-29 [decisions/proposal-seat-capability-manifest.md]; **format-shape RESOLVED** — roledef SCHEMA v0.3.0 shipped `recommended_capabilities[]` on Role+Job, independent re-derivation of orgdef-strategist's 2026-05-17 proposal, so now **2 independent derivations + shipped format**; promotion still held until first OAGP seats carry capabilities + bind/onboard exercise them). Promotion is ongoing via the promotion-follows-adoption discipline as patterns earn it.
- **Seat-capability-manifest wiring** (forward, implementer-execution when seats specialize) — roledef `recommended_capabilities[]` (v0.3.0, kind enum covers skills/plugins/MCP) is the encoding; orgdef:Position inherits transitively (no orgdef change); on authoring OAGP seats' deferred roledef:Job items, declare capabilities → /oagp-onboard surfaces them → bind() references them; completes pattern #6 (substrate-is-sufficient-agent-context) and is the validation that earns the promotion
